package servlet;

import java.io.IOException;
import java.util.ArrayList;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import model.CheckStockData;
import model.CheckStockMethod;

@WebServlet("/CheckStock")
public class CheckStock extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		ArrayList<CheckStockData> checkStockData = new ArrayList<CheckStockData>();
		CheckStockMethod csm = new CheckStockMethod();
		checkStockData = csm.execute();
		request.setAttribute("checkStockData", checkStockData);
		RequestDispatcher rd = request.getRequestDispatcher("checkStock.jsp");
        rd.forward(request, response);
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		doGet(request, response);
	}

}
